      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-xs font-medium tracking-wide">
          {['FEATURES', 'WORKFLOW', 'PRICING'].map((link) => (
            <a key={link} href={`#${link.toLowerCase()}`} className={`transition-colors ${mounted && theme === 'light' ? 'text-slate-600 hover:text-slate-900' : 'text-white/40 hover:text-white'}`}>
              {link}
            </a>
          ))}
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/dashboard" className={`px-4 py-2 border text-xs font-medium transition-all ${mounted && theme === 'light' ? 'border-slate-200 text-slate-700 hover:bg-slate-50' : 'border-white/10 text-white/60 hover:bg-white/5 hover:text-white'}`}>
            ACCESS CONSOLE
          </Link>
        </div>
      </div>
